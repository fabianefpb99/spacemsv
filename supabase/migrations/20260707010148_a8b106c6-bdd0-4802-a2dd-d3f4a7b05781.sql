-- ============================================================
-- Rebalance de RTP: Slot Mafia, Slot Samurai, Arena
-- Sólo CREATE OR REPLACE de funciones existentes. Se preservan
-- SECURITY DEFINER, search_path y GRANTs previos.
-- ============================================================
...
DECLARE
  ...
  -- Ajuste 2026-07-07: sube pagos 3-de-un-tipo en símbolos bajos (watch..card)
  -- para dar más sensación de victoria manteniendo ventaja de casa.
...
DECLARE
  v_symbol_ids     text[] := ARRAY['boss','car','brief','gold','watch','chip','hat','card'];
  v_symbol_weights int[]  := ARRAY[2,3,4,5,6,8,10,12];
  -- Ajuste 2026-07-07: sube pagos de 3 y 4 iguales para levantar el RTP real
  -- (venía en ~45%) y dar más sensación de victoria. Pay5 sin cambio.
  ...
  v_bet_tx_id uuid; v_grid int[][]; v_grid_jsonb jsonb; v_reel_jsonb jsonb;
  v_wins jsonb := '[]'::jsonb; v_cells jsonb; v_total int := 0;
...
BEGIN
  ...
  IF v_boost AND v_total = 0 THEN
    ...
    FOR k IN 1..v_boost_count LOOP
      v_cells := v_cells || jsonb_build_array(jsonb_build_array(k - 1, v_boost_row));
    END LOOP;
    v_wins := jsonb_build_array(jsonb_build_object('lineIdx', 0,
      'symbolId', v_symbol_ids[v_boost_sym + 1], 'count', v_boost_count,
      'payout', v_payout, 'cells', v_cells));
    v_total := v_payout;
  END IF;
...
-- ---------- Arena ----------
-- Ajuste 2026-07-07: baja multiplicadores base y sube el peso de la casa a 10%
-- para cerrar la ventaja del jugador. RTP objetivo por personaje: ~86% (nova/shadow/titan)
-- y ~58% (blaze). Antes: nova/etc pagaban 3.30 con 28% win = 92%; blaze 8.00 con
-- 11% = 88%; house sólo 5% → RTP inflado.
...
DECLARE
  ...
  v_base_odds    numeric[] := ARRAY[3.20, 3.20, 3.20, 6.50];
  v_base_weights int[]     := ARRAY[270, 270, 270, 90];
  v_odds  numeric[];
  v_weights int[];
  v_total_w int := 1000;
  v_house_w int := 100;
...
BEGIN
  ...
  v_weights := ARRAY[
    v_base_weights[v_perm[1]], v_base_weights[v_perm[2]],
    v_base_weights[v_perm[3]], v_base_weights[v_perm[4]]
  ];
  v_odds := ARRAY[
    v_base_odds[v_perm[1]], v_base_odds[v_perm[2]],
    v_base_odds[v_perm[3]], v_base_odds[v_perm[4]]
  ];
  ...
  IF v_pick >= (v_total_w - v_house_w) THEN
    ...
    v_rivals := ARRAY[]::int[];
    FOR k IN 1..4 LOOP
      IF k <> v_bet_idx THEN v_rivals := array_append(v_rivals, k); END IF;
    END LOOP;
  ...
  ELSE
    ...
    FOR k IN 1..array_length(v_weights, 1) LOOP
      ...
      IF v_pick < v_acc THEN v_winner_idx := k; EXIT; END IF;
    END LOOP;
  END IF;
  ...
  IF v_won THEN v_payout := ROUND(p_bet_amount * v_multiplier); END IF;
  ...
  FOR i IN 1..v_rounds_n LOOP
    ...
    FOR k IN 1..4 LOOP
      IF v_hp[k] > 0 THEN
        ...
        IF k <> v_winner_idx THEN v_non_winner_alive := array_append(v_non_winner_alive, k); END IF;
      END IF;
    END LOOP;
    ...
    IF COALESCE(v_target_idx, 0) = 0 THEN
      ...
      FOR k IN 1..array_length(v_alive, 1) LOOP
        IF v_alive[k] <> v_atk_idx THEN
          m := m - 1;
          IF m = 0 THEN v_target_idx := v_alive[k]; EXIT; END IF;
        END IF;
      END LOOP;
    END IF;

    IF v_target_idx = 0 THEN CONTINUE; END IF;
    ...
    v_log_arr := v_log_arr || jsonb_build_array(jsonb_build_object(
      'round', i, 'attacker', v_chars[v_atk_idx], 'target', v_chars[v_target_idx], 'damage', v_damage,
      'hp', jsonb_build_object('nova', v_hp[1], 'shadow', v_hp[2], 'titan', v_hp[3], 'blaze', v_hp[4])
    ));
  END LOOP;
  ...
  WHILE v_alive_count > 2 AND v_non_winner_kills < 2 LOOP
    v_non_winner_alive := ARRAY[]::int[];
    FOR k IN 1..4 LOOP
      IF v_hp[k] > 0 AND k <> v_winner_idx THEN v_non_winner_alive := array_append(v_non_winner_alive, k); END IF;
    END LOOP;
    ...
    v_log_arr := v_log_arr || jsonb_build_array(jsonb_build_object(
      'round', 9000 + v_non_winner_kills, 'attacker', v_chars[v_atk_idx], 'target', v_chars[v_target_idx], 'damage', v_damage,
      'hp', jsonb_build_object('nova', v_hp[1], 'shadow', v_hp[2], 'titan', v_hp[3], 'blaze', v_hp[4])
    ));
  END LOOP;
  ...
  FOR k IN 1..4 LOOP
    IF k <> v_winner_idx AND v_hp[k] > 0 THEN
      ...
      v_hp[k]  := 0;
      v_log_arr := v_log_arr || jsonb_build_array(jsonb_build_object(
        'round', 9500 + k, 'attacker', v_chars[v_winner_idx], 'target', v_chars[k], 'damage', v_damage, 'finisher', true,
        'hp', jsonb_build_object('nova', v_hp[1], 'shadow', v_hp[2], 'titan', v_hp[3], 'blaze', v_hp[4])
      ));
    END IF;
  END LOOP;
  ...
  v_round_payload := jsonb_build_object(
    'character_bet', p_character, 'winner', v_winner, 'won', v_won,
    'multiplier', v_multiplier, 'payout', v_payout, 'bet_amount', p_bet_amount,
    'odds_snapshot', jsonb_build_object('nova', v_odds[1], 'shadow', v_odds[2], 'titan', v_odds[3], 'blaze', v_odds[4]),
    'combat_log', v_combat_log, 'server_seed', v_server_seed, 'server_seed_hash', v_server_seed_hash
  );
  ...
  INSERT INTO public.transactions (user_id, type, amount, balance_after, game, client_action_id, meta)
  VALUES (p_user_id, 'bet'::transaction_type, -p_bet_amount, v_new_balance, 'arena', p_client_action_id,
    jsonb_build_object('kind','arena_bet','server_seed_hash',v_server_seed_hash,'server_seed',v_server_seed,
      'bet_amount',p_bet_amount,'character',p_character,'from_bonus',v_from_bonus,'from_real',v_from_real,
      'result',v_round_payload,'round_id',v_round_id))
  RETURNING id INTO v_bet_tx_id;

  IF v_payout > 0 THEN
    SELECT cw.new_balance INTO v_new_balance FROM public._credit_win(p_user_id, v_payout) cw;
    INSERT INTO public.transactions (user_id, type, amount, balance_after, game, client_action_id, meta)
    VALUES (p_user_id, 'win'::transaction_type, v_payout, v_new_balance, 'arena', NULL,
      jsonb_build_object('kind','arena_win','bet_tx_id',v_bet_tx_id,'bet_action_id',p_client_action_id,
        'multiplier',v_multiplier,'winner',v_winner,'to_real',v_payout,'round_id',v_round_id));
  END IF;

  RETURN jsonb_build_object('was_duplicate', false, 'new_balance', v_new_balance,
    'round_id', v_round_id, 'cached', v_round_payload);
END;
$function$;

-- ---------- Ruleta: bajar RTP subiendo peso del verde ----------
-- Cambiar el dato en roulette_config: green_weight 1.9 → 2.5
-- (RTP rojo/negro pasa de ~97.3% a ~93.2%). Es sólo data, pero lo
-- aplicamos aquí para dejar el rebalanceo en una sola migración auditable.
UPDATE public.roulette_config SET green_weight = 2.5, updated_at = now();

-- ---------- Actualizar targets de RTP para reflejar el nuevo diseño ----------
UPDATE public.game_rtp_config SET rtp_target = 93.2  WHERE game = 'ruleta';
UPDATE public.game_rtp_config SET rtp_target = 82.0  WHERE game = 'arena';
UPDATE public.game_rtp_config SET rtp_target = 35.0  WHERE game = 'dice';
UPDATE public.game_rtp_config SET rtp_target = 82.0  WHERE game = 'slot';
UPDATE public.game_rtp_config SET rtp_target = 88.0  WHERE game = 'slot_samurai';
UPDATE public.game_rtp_config SET rtp_target = 97.5  WHERE game = 'blackjack';