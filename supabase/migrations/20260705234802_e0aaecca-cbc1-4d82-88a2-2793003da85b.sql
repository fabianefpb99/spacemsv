...
DECLARE
  ...
  -- Pagos de 3 iguales rebajados en símbolos bajos (watch/chip/hat/card)
  -- para dar hits pequeños frecuentes por debajo de la apuesta.
  v_pay3           int[]  := ARRAY[55,34,24,19,9,6,5,4];
...
BEGIN
  ...
  -- Evaluación con Pay Both Ways (L→R y R→L)
  FOR i IN 1..v_line_count LOOP
    ...
    -- Derecha a izquierda (saltar si L→R ya cubrió los 5 carretes)
    IF v_cnt_l >= v_reels THEN
      CONTINUE;
    END IF;
    ...
    IF v_cnt_r >= 3 THEN
      ...
      IF v_payout > 0 THEN
        v_cells := '[]'::jsonb;
        FOR k IN 1..v_cnt_r LOOP
          v_cells := v_cells || jsonb_build_array(jsonb_build_array(v_reels - k, v_paylines[i][v_reels - k + 1]));
        END LOOP;
        v_wins := v_wins || jsonb_build_array(jsonb_build_object(
  ...
  -- Débito de apuesta y crédito de premio
  SELECT balance INTO v_current_balance
  ...
  v_new_balance := v_current_balance - p_bet_amount + v_total;
  UPDATE public.user_balances
     SET balance = v_new_balance, updated_at = now()
   WHERE user_id = p_user_id;

  -- Construcción del grid jsonb como matriz de reels
  v_grid_jsonb := '[]'::jsonb;
  FOR i IN 1..v_reels LOOP
    v_reel_jsonb := '[]'::jsonb;
    FOR j IN 1..v_rows LOOP
  ...
  INSERT INTO public.transactions(user_id, kind, amount, client_action_id, meta)
  VALUES (p_user_id, 'slot_samurai_spin', v_total - p_bet_amount, p_client_action_id, jsonb_build_object(
