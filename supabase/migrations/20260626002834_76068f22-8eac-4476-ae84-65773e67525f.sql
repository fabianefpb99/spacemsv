...
AS $$
  ...
  with_refs AS (
    SELECT e.*,
           bt.amount AS bet_tx_amount,
           gb.payout AS gb_payout,
           gb.amount AS gb_amount
    FROM enriched e
    LEFT JOIN public.transactions bt
      ON bt.id = e.bet_tx_id AND bt.type = 'bet'
    LEFT JOIN public.game_bets gb
      ON gb.round_id = e.game_round_id AND gb.user_id = e.user_id
  )
  SELECT w.user_id,
