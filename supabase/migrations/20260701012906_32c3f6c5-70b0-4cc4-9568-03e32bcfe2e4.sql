...
-- Restore v1 functions that v2 depends on. Keep them locked down:
-- clients still call v2 (which uses auth.uid()); v1 is only reachable
-- via the SECURITY DEFINER call chain from v2.
...
BEGIN
  ...
  RETURN jsonb_build_object(
    'was_duplicate', false, 'new_balance', v_new_balance,
    'round_id', v_round_id, 'cached', v_round_payload
  );
END;
$function$;

-- Lock down: only reachable from v2 (SECURITY DEFINER chain).
...
BEGIN
  ...
  IF v_segment = 0 THEN v_winning_color := 'green';
  ELSIF v_segment = ANY(v_red_numbers) THEN v_winning_color := 'red';
  ELSE v_winning_color := 'black';
  END IF;
  ...
  v_result := jsonb_build_object(
    'winning_segment', v_segment, 'winning_color', v_winning_color, 'choice', p_choice,
    'won', v_won, 'multiplier', v_multiplier, 'payout', v_payout, 'bet_amount', p_bet_amount,
    'server_seed', v_server_seed, 'server_seed_hash', v_server_seed_hash
  );

  INSERT INTO public.transactions (user_id, type, amount, balance_after, game, client_action_id, meta)
  VALUES (p_user_id, 'bet'::transaction_type, -p_bet_amount, v_new_balance, 'ruleta', p_client_action_id,
    jsonb_build_object('kind','roulette_bet','server_seed_hash',v_server_seed_hash,'server_seed',v_server_seed,
      'bet_amount',p_bet_amount,'choice',p_choice,'from_bonus',v_from_bonus,'from_real',v_from_real,'result',v_result))
  RETURNING id INTO v_bet_tx_id;

  IF v_payout > 0 THEN
    SELECT cw.new_balance INTO v_new_balance FROM public._credit_win(p_user_id, v_payout) cw;
    INSERT INTO public.transactions (user_id, type, amount, balance_after, game, client_action_id, meta)
    VALUES (p_user_id, 'win'::transaction_type, v_payout, v_new_balance, 'ruleta', NULL,
      jsonb_build_object('kind','roulette_win','bet_tx_id',v_bet_tx_id,'bet_action_id',p_client_action_id,
        'multiplier',v_multiplier,'winning_segment',v_segment,'winning_color',v_winning_color,'to_real',v_payout));
  END IF;

  RETURN jsonb_build_object('was_duplicate', false, 'new_balance', v_new_balance, 'cached', v_result);
END;
$function$;
