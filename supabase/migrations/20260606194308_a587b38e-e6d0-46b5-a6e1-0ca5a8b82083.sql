...
BEGIN
  ...
  -- Combat log: 8-12 rounds, uniform-random attacker/target among living
  -- fighters. Winner is shielded from lethal damage during the loop so the
  -- finisher events are never authored by a dead fighter.
  v_hash_bytes := digest(v_server_seed || '|combat', 'sha256');
  v_rounds_n   := 8 + (get_byte(v_hash_bytes, 0) % 5);
  j := 1;
  v_alive_count := 4;
  ...
  -- Finisher: winner kills whoever is still standing.
  FOR k IN 1..4 LOOP
