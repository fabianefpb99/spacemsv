...
DECLARE
  ...
  -- Opción A: 3 comunes (3.30x, 28%) + 1 estrella (8.00x, 11%). House 5%.
...
BEGIN
  ...
  INSERT INTO public.transactions (
    user_id, type, amount, balance_after, game, game_round_id, client_action_id, meta
  ) VALUES (
    p_user_id, 'bet'::transaction_type, -p_bet_amount, v_new_balance, 'arena', v_round_id, p_client_action_id,
    jsonb_build_object(
      'kind',             'arena_bet',
      'round_id',         v_round_id,
      'server_seed_hash', v_server_seed_hash,
  ...
  IF v_payout > 0 THEN
    INSERT INTO public.transactions (
      ...
      p_user_id, 'win'::transaction_type, v_payout, v_new_balance, 'arena', v_round_id, NULL,
      jsonb_build_object(
        'kind',          'arena_win',
        'round_id',      v_round_id,
