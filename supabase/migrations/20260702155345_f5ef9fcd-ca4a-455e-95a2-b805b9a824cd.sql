...
BEGIN
  ...
  -- Rejection sampling con 2 bytes (0..65535). Calculamos el múltiplo más alto de v_total_weight
  -- que no exceda 65536 para mantener distribución uniforme exacta.
  v_rejection_limit := v_total_weight * (65536 / v_total_weight);

  LOOP
    v_byte_hi := get_byte(gen_random_bytes(1), 0);
    v_byte_lo := get_byte(gen_random_bytes(1), 0);
    v_random_int := (v_byte_hi << 8) | v_byte_lo;
  ...
  ELSIF v_pick < v_red_threshold THEN
    v_byte_lo := get_byte(gen_random_bytes(1), 0);
    v_segment := v_red_numbers[1 + (v_byte_lo % array_length(v_red_numbers, 1))];
    v_winning_color := 'red';
  ELSE
    v_byte_lo := get_byte(gen_random_bytes(1), 0);
    v_segment := v_black_numbers[1 + (v_byte_lo % array_length(v_black_numbers, 1))];
  ...
  IF v_boost AND NOT v_won AND p_choice IN ('red','black') THEN
    ...
    v_byte_lo := get_byte(gen_random_bytes(1), 0);
    v_segment := v_choices[1 + (v_byte_lo % array_length(v_choices, 1))];
