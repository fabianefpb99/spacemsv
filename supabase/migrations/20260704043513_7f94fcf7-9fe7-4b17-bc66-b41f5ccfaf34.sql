...
-- 1) Hide admin UUID: revoke column-level SELECT on updated_by (and updated_at metadata) from public reads
REVOKE SELECT (updated_by) ON public.vip_rank_rewards FROM anon, authenticated;

-- 2) Lock username, first_name, last_name, email in profiles update policy
...
-- 3) Restrict SECURITY DEFINER public.has_role from anon
